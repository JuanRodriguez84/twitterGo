...
func EjecutoLambda(ctx context.Context, request events.APIGatewayProxyRequest) (*events.APIGatewayProxyResponse, error) { // entre parentesis colocar los parametros que va a recibir
	...
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("password"), SecretModel.Password) // en secretmanager está como L3Qzbh6IklAiZFDv
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("host"), SecretModel.Host)         // en secretmanager está	twitter.hesfakk.mongodb.net que es la URL que da Mongo compass para conectarnos a la BD de mongo
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("database"), SecretModel.Database) // en secretmanager está como twitter
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("jwtSign"), SecretModel.JWTSign)   // en secretmanager está como MastersdelDesarrollo_grupodeFacebook
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("body"), request.Body)
	awsgo.Ctx = context.WithValue(awsgo.Ctx, models.Key("bucketName"), os.Getenv("BucketName")) // las imagenes bienen codificadas en base 64 pero vienen como texto. Variables de entorno de la lambda AWS esta como "twitterGo"
